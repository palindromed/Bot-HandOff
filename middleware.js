module.exports = {
    ...
    incoming: function (message, bot, builder) {
        ...
        } else if (!global.users.includes(message.conversation.id)) {

            global.users.push(message.conversation.id);

        };
        if (!message.user.isStaff) {
            global.conversations[message.conversation.id] = Object.assign({}, global.conversations[message.conversation.id], {  transcript: [message],
