...
bot.dialog('/callCenterConversation', [
    (session, result) => {
        ...
        if (session.message.text === 'break') {
            ...
            session.endDialog();
        } else if (session.message.text === 'transcript') {
            // Give call center the ability to retrieve transcript from user interaction with bot
            // Won't send 'transcript' to user
            storage.getTranscript(session.privateConversationData.contactInfo.conversation.id, session.privateConversationData.contactInfo.user.id, (transcript) => {
                session.send('Here is the previous conversation the user had with the bot');
                session.send('Transcript: ' + transcript);
            });
        } else {
...
/*
For this example, only text sent between the bot and user is transcribed
Look into built in support for zipping conversation for more complex scenarios
*/
let transcribeConversation = (session) => {
    let botText = '';
    for (let i = 0; i < session.batch.length; i++) {
        botText += session.batch[i].text + ', ';
    }
    return botText;
}